...
interface RegionBasedFreePriceProps {
  className?: string;
}

export default function RegionBasedFreePrice({ 
  className = "" 
}: RegionBasedFreePriceProps) {
  ...
  return (
    <p className={`text-3xl font-bold mb-6 ${className}`}>
      {currencySymbol}0 <span className="text-base font-normal text-muted-foreground">forever</span>
    </p>
  );
}
