...
interface RegionBasedPricingProps {
  euPrice: string;
  nonEuPrice: string;
  period: string;
  className?: string;
...
export default function RegionBasedPricing({ 
  euPrice, 
  nonEuPrice, 
  period,
  className = "" 
}: RegionBasedPricingProps) {
  ...
  return (
    <p className={`text-lg font-medium mt-2 ${className}`}>
      or {currencySymbol}{displayPrice} <span className="text-base font-normal text-muted-foreground">/ {period} (2 months free)</span>
    </p>
  );
}
